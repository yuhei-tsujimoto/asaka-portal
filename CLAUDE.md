...
- **ファイル名がタイトルになる:** Markdownファイル内で見出し1（`#`）を使用しない
    - ファイル名（または`zensical.toml`で設定されたタイトル）がナビゲーションメニューに表示される
    - ページ内のコンテンツは見出し2（`##`）から開始する

- **ファイル名が並び順を決定する:** `nav_order:` frontmatterは使用しない
    - ファイルはディレクトリ内のファイル名順（ファイルシステムの順序）に自動的に並ぶ
    - 番号付きのファイル名（例：`01_`, `02_`など）を使用して順序を制御する
